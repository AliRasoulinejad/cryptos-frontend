...
export function FourOhFourCategoryCard(blog) {
    return (
        <div className="col-xl-4 col-lg-6 col-md-6 mb-4">
            <div className="card card-custom">
                <Link href={blog.url} className="post-link">
                    <Image className="card-img-top thumbnail card-img-top-custom" src={blog.image} alt={blog.title} />
                </Link>
                <div className="card-body card-block-custom">
                    <div className="card-block card-block-custom card-block-post-custom">
                        <div>
                            <h2 className="card-title"><Link href={blog.url}>{blog.title}</Link></h2>
                            <p className="card-text">{blog.text_index}</p>
                        </div>
                        ...
                        <div>
                            <div className="wrapfooter">
                                <span className="meta-footer-thumb">
                                    <Link href={blog.author.url}>
                                        <Image className="author-thumb" src={blog.author.get_image_url} alt={blog.author.name} />
                                    </Link>
                                </span>
                                <span className="author-meta">
                                    ...
                                    <br/>
                                    <span className="post-date" created_at={blog.created_at}>{blog.created_at}</span>
                                    <span className="dot"></span>
                                    <span className="post-read">read at {blog.reading_time} minutes</span>
                                </span>
                            </div>
                            <hr/>
                            ...
                            <div className="show-like-dislike-comment">
                                ...
                                <span className="show-dislike-detail">
                                    ...
                                    <span>{blog.dis_likes_count}</span>
                                </span>
